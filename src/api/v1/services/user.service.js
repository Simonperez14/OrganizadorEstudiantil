...
export const upgradePlan = async (userId) => {
    ...
    // descomentar si es necesario ser admin para cambiar de plan
    /*
    if (usuario.role !== "estudiante") {
        const error = new Error("Los usuarios admin no gestionan planes");
        error.status = 400;
        throw error;
    }*/

    if (usuario.plan !== "plus") {
        const error = new Error("Solo se puede hacer upgrade desde el plan plus");
        error.status = 400;
        throw error;
    }
